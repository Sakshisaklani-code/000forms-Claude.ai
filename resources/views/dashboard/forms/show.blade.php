...
<div class="page-header">
    <div>
        <a href="{{ route('dashboard') }}" class="text-muted" style="font-size: 0.875rem; display: inline-flex; align-items: center; gap: 0.5rem; margin-bottom: 0.5rem;">
...
@if(!$form->email_verified)
    <div class="alert alert-warning">
        ...
        <div style="flex: 1;">
            <strong>Email verification required.</strong> Please check {{ $form->recipient_email }} and click the verification link.
        </div>
...
<div class="card mb-3">
    ...
    <div class="endpoint-display">
        ...
        <span class="endpoint-url">{{ $form->endpoint_url }}</span>
...
<span class="tag">&lt;form</span> <span class="attr">id</span>=<span class="string">"form-{{ $form->slug }}"</span> <span class="attr">action</span>=<span class="string">"{{ $form->endpoint_url }}"</span> <span class="attr">method</span>=<span class="string">"POST"</span><span class="tag">&gt;</span>
  <span class="tag">&lt;input</span> <span class="attr">type</span>=<span class="string">"email"</span> <span class="attr">name</span>=<span class="string">"email"</span> <span class="attr">placeholder</span>=<span class="string">"Your email"</span> <span class="attr">required</span><span class="tag">&gt;</span>
  <span class="tag">&lt;textarea</span> <span class="attr">name</span>=<span class="string">"message"</span> <span class="attr">placeholder</span>=<span class="string">"Your message"</span><span class="tag">&gt;&lt;/textarea&gt;</span>
...
<span class="keyword">document</span>.getElementById(<span class="string">'form-{{ $form->slug }}'</span>).addEventListener(<span class="string">'submit'</span>, <span class="keyword">async function</span>(e) {
    ...
    <span class="keyword">const</span> responseBox = <span class="keyword">document</span>.getElementById(<span class="string">'form-response-{{ $form->slug }}'</span>);
    ...
    <span class="keyword">try</span> {
        ...
        <span class="keyword">if</span> (response.ok &amp;&amp; data.success) {
            responseBox.innerHTML = <span class="string">'&lt;span style="color: #22c55e;"&gt; {{ $form->success_message ?? 'Thank you! Your message has been sent.' }}&lt;/span&gt;'</span>;
            form.reset();
...
<span class="tag">&lt;form</span> <span class="attr">action</span>=<span class="string">"{{ $form->endpoint_url }}"</span> 
...
<div class="stats-grid">
    ...
    <div class="card stat-card">
        ...
        <div class="stat-value">{{ number_format($form->spam_count) }}</div>
    </div>
    <div class="card stat-card">
        <div class="stat-label">Last Submission</div>
        <div class="stat-value" style="font-size: 1.25rem;">{{ $form->last_submission_at ? $form->last_submission_at->diffForHumans() : 'Never' }}</div>
    </div>
</div>
            ...
            <tbody>
                @foreach($submissions as $submission)
                    <tr>
                        ...
                        <td class="text-muted">{{ $submission->created_at->format('M j, Y g:i A') }}</td>
                        <td>
                        ...
                        <td class="text-right">
                            <a href="{{ route('dashboard.submissions.show', [$form->id, $submission->id]) }}" class="btn btn-ghost btn-sm">
                                View
                            </a>
                        </td>
                    </tr>
